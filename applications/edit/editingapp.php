...
if (isset($_POST['submit']) or isset($_SESSION['idapp'])) {
    $idapp  = mysqli_real_escape_string($conn, $_SESSION['idapp']);
    $edit   = mysqli_real_escape_string($conn, $_POST['edit']);
    if (empty($_POST['edit']) and empty($_SESSION['idapp'])) error("Variable vide", "../applications/edit/editapp.php");
    else {
        if (!$conn->connect_errno) {
            $res = $conn->query("SELECT icone, nom, id FROM chb_applications WHERE nom = \"$edit\" OR id = \"$idapp\"");
            $row = $res->fetch_assoc();
        }
    }
}
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Modifier l'application <?php echo htmlspecialchars($row['nom']); ?></h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="#">Accueil</a></li>
                                <li class="breadcrumb-item active">Modifier une application</li>
                            </ol>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
            ...
            <div class="content d-flex justify-content-center">
                ...
                <div class="card card-warning w-25">
                    <div class="card-header">
                        <h3 class="card-title">Modification de l'application</h3>
                    </div>
                    <!-- form start -->
                    <form role="form" action="editappthink.php" method="POST" enctype="multipart/form-data">
                        <input type="hidden" name="oldid" value="<?php echo htmlspecialchars($row['id']); ?>" />
                        <div class="card-body">
                            <div class="form-group">
                                <label for="fileToUpload">Icône de l'application :</label>
                                <div class="input-group">
                                    <div class="custom-file">
                                        <input type="file" class="custom-file-input" id="fileToUpload" name="fileToUpload">
                                        <label class="custom-file-label" for="fileToUpload"><?php echo htmlspecialchars($row['icone']); ?></label>
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Nom de l'application :</label>
                                <input type="text" class="form-control" name="nom" placeholder="Nom de l'application" required value="<?php echo htmlspecialchars($row['nom']); ?>">
                            </div>
                        </div>
                        <!-- /.card-body -->
