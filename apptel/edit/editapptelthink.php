...
if (isset($_POST['submit'])) {
    ...
    if (empty($oldid)) error("Variable vide", "../telephones/listetel.php");
    else {
        if (empty($cron)) $cron =  "0 12 * * *";
        $sql = mysqli_prepare($conn, "UPDATE chb_apptel SET editedat = NOW(), endpointARN = ?, cron = ?, message = ? WHERE id = ?");
        mysqli_stmt_bind_param($sql, 'sssi', $endpointARN, $cron, $message, $oldid);
        if (mysqli_stmt_execute($sql)) success("Le lien est modifié", "");
