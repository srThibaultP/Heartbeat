...
if (!$conn->connect_errno) $res = $conn->query("SELECT * FROM chb_telephones");
?>
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <div class="row">
                                <h1 class="col-4 m-0">Liste des téléphones</h1>
                                <form class="col-4" action="../telephones/add/addtel.php" method="POST">
                                    <input type="hidden" name="idtel" value="' . $row['id'] . '" />
                                    <button type="submit" class="btn bg-primary"><i class="fas fa-plus-circle"></i> Ajouter un téléphone</button>
                                </form>
                            </div>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                ...
                                <li class="breadcrumb-item active">Liste des téléphones</li>
                            </ol>
                        </div><!-- /.col -->
                    </div><!-- /.row -->
                </div><!-- /.container-fluid -->
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    while ($row = $res->fetch_assoc()) {
                                        echo '<td><div class="row align-items-start"><form action="../telephones/edit/edittel.php" method="POST">';
                                            echo '<input type="hidden" name="idtel" value="' . htmlspecialchars($row['id']) . '"/>';
                                        echo '<button type="submit" class="btn bg-warning"><i class="fas fa-wrench"></i></button></form>';
                                        
                                        echo '<form action="../telephones/rm/rmtel.php" method="POST">';
                                            echo '<input type="hidden" name="idtel" value="' . htmlspecialchars($row['id']) . '"/>';
                                        ...
                                        echo '<form action="../apptel/add/linkapptel.php" method="POST">';
                                            echo '<input type="hidden" name="idtel" value="' . htmlspecialchars($row['id']) . '"/>';
                                        echo '<button type="submit" class="btn btn-app bg-success"><i class="fas fa-link"></i>Lier</button></form>';

                                        echo '<form action="../apptel/edit/editapptel.php" method="POST">';
                                        echo '<input type="hidden" name="idtel" value="' . htmlspecialchars($row['id']) . '"/>';
                                            echo '<button type="submit" class="btn btn-app bg-info"><i class="fas fa-edit"></i>Editer</button></form>';

                                        echo '<form action="../apptel/rm/rmapptel.php" method="POST">';
                                        echo '<input type="hidden" name="idtel" value="' . htmlspecialchars($row['id']) . '"/>';
                                            echo '<button type="submit" class="btn btn-app bg-danger"><i class="fas fa-unlink"></i>Dissocier</button></form>';
                                        echo "</td></tr>";
                                    } ?>
                            </tbody>
                        </table>
