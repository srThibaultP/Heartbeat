...
if (isset($_POST['submit'])) {
    ...
    if (empty($oldid)) error("Variable oldid vide", "../telephones/edit/edittel.php");
    else {
        $sql = mysqli_prepare($conn, "UPDATE chb_telephones SET nom = ?, os = ? WHERE id = ?");
        mysqli_stmt_bind_param($sql, 'ssi', $nom, $os, $oldid);
        if (mysqli_stmt_execute($sql)) success("Le téléphone est modifié !", "../telephones/listetel.php");
        else error(mysqli_error($conn), "../telephones/edit/edittel.php");
    }
}
?>
<!DOCTYPE html>
