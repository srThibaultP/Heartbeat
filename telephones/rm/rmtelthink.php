...
if (isset($_POST['submit'])) {
    ...
    if (empty($supp)) error("Variable supp vide", "../telephones/rm/rmtel.php");
    else {
        $sqllocatetelid = $conn->query("SELECT id FROM chb_telephones WHERE nom = \"$supp\"");
        $sqllocatetelid->data_seek(0);
        $sqltelid = $sqllocatetelid->fetch_assoc();
        $sqlteltel = "DELETE FROM chb_apptel WHERE idtel = \"$sqltelid[id]\"";
        $sql = "DELETE FROM chb_telephones WHERE nom = \"$supp\"";
        if (mysqli_query($conn, $sql) and mysqli_query($conn, $sqlteltel)) success("Le téléphone est supprimé.", "../telephones/listetel.php");
        else error(mysqli_error($conn), "../telephones/rm/rmtel.php");
    }
}
?>
<!DOCTYPE html>
